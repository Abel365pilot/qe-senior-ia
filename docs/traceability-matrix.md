...
| E2 | README reproducible, variables, proveedor y cuota | README raíz y por bloque; `.env.example`; SUT fijado por commit/OpenAPI SHA | **Cumplido** en instrucciones; primera ejecución Docker del workflow aislado aún pendiente |
...
|---|---|---|---|---|
| N1 Funcionalidad | ¿CA 1-3 y el negativo funcionan en un SUT aislado? | 4 escenarios; contratos; 18 auxiliares; 4/4 diagnóstico público; workflow Docker fijado | HTTP, contrato, semántica y cleanup | **Parcial** hasta el primer run verde del workflow Docker; no se confunde el stub hermético con Toolshop |
...
| O7 Intermitencia | ¿Diagnostica sin retry ciego? | Carrera catálogo→carrito, `X-QE-Run-Id`, body/status preservados, SUT fijado | Fallo conserva causa; una limpieza defensiva | **Cumplido en diseño**; ejecución Docker pendiente |
...
| P12 Documentación | ¿Un tercero puede reproducir/auditar? | Versiones, SUT/OpenAPI SHA, comandos, CI, manifiesto, SDK sanitizado | `scripts/release_gate.py` | **Parcial** hasta el primer run Docker verde |
...
|---|---|---|---|
| 1. Login válido e inválido | Karate escenarios 1 y 2 | 4/4 diagnóstico público; Docker workflow | **Parcial** hasta run Docker |
| 2. Buscar/filtrar catálogo | Karate escenario 3 | Contrato paginado, texto y categoría | **Parcial** hasta run Docker |
| 3. Cantidad/total carrito | Karate escenario 4 + unidades | Cantidad 2, descuentos, `BigDecimal`, cleanup | **Parcial** hasta run Docker |
...
- **NO-GO — liberación productiva:** se requiere run Docker verde contra la
  revisión fijada, 35 casos como mínimo (50 recomendados) por segmento,
  etiquetas humanas y calibración del juez.
