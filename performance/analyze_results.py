...
def _history(history_path: Path) -> dict[str, Any]:
    ...
    baseline_p95 = p95_values[0] if p95_values else None
    first_double_baseline_row = next(
        (
            row
            for row in active
            if baseline_p95 is not None
            and row.get("95%") not in (None, "", "N/A")
            and numeric(row, "95%") >= baseline_p95 * 2
        ),
        None,
    )
    ...
    return {
        ...
        "first_p95_at_least_2x_baseline": observation(first_double_baseline_row),
...
def summarize(prefix: Path, profile: str) -> dict[str, Any]:
    ...
    summary = {
        ...
        "history": _history(Path(f"{prefix}_stats_history.csv")),
    }
    return summary
    ...
    if history.get("available"):
        lines.extend(
            [
                ...
                f"- p95 base/máximo: {history['baseline_p95_ms']} / {history['maximum_p95_ms']} ms",
                ...
                f"- Primer p95 >=2x baseline: {history['first_p95_at_least_2x_baseline']}",
            ]
        )
